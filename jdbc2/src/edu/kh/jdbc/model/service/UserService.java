...
public class UserService {
	 ...
	 * @return insert된 결과행의 갯수->view로 돌아감
	 */
	public int insertUser(User user) throws Exception {
		...
		// 4.DML(INSERT) 수행결과에 따라 트랜젝션 제어처리
	...
	/**
	 * 3.User 중 이름에 검색어가 포함된 회원조회서비스
	 * 
	 * @param keyword:입력받은 키워드
	 * @return searchList:조회된 회원리스트
	 */
	...
	/**
	 * 4.USER_NO를 입력받아 일치하는 USER 조회서비스
	 * 
	 * @param input
	 * @return
	 */
	...
	public int selectUserNo(String userId, String userPw) throws Exception {
		Connection conn = JDBCTemplate.getConnection();
		int userNo = dao.seletUser(conn, userId, userPw);
		JDBCTemplate.close(conn);
		return userNo;
	}

	/**
	 * 6-2.USER_NO가 일치하는 회원의 이름수정서비스(UPDATE)
	 * 
	 * @param name
	 * @param userNo
	 * @return
	...
	/**
	 * userList에 있는 모든 User 객체를 INSERT 하는 서비스
	 * 
	 * @param userList
	 * @return
	 */
	public int multiInsertUser(List<User> userList) throws Exception {
		...
		Connection conn = JDBCTemplate.getConnection();
		int count = 0; // 삽입성공한 행의 갯수 count

		for (User user : userList) {
			int result = dao.insertUser(conn, user);
			count += result; // 삽입 성공한 행의 갯수를 count에 누적
		}
		...
		JDBCTemplate.close(conn);
		return count; // result는 for문안에 만들어진 지역변수
	}
