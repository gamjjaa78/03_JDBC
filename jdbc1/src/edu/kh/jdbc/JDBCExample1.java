...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		//Java에서 DB에 연결할 수 있게 해주는 Java API(Java에서 제공하는 코드)
		//->java.sql 패키지에 존재
		...
		//******java.sql.Statement
		//-1)SQL을 Java->DB에 전달
		//-2)DB에서 SQL 수행한 결과를 반환받아옴(DB->Java)
		Statement stmt=null;
		...
		try {
		//2. DriverManager 객체이용해 Connection 객체 생성하기
		//java.sql.DriverManager
		//-DB연결정보와 JDBC 드라이버를 이용해서
		...
		//2-2)DB연결정보 작성
			...
			String host="localhost"; //DB 서버 컴퓨터의 IP 또는 도메인주소, localhost==현재컴퓨터
			String port=":1521"; //DB 서버에 연결하기 위한 PORT 번호
			String dbName=":XE"; //DBMS 이름 (xe==eXpress Edition)
			//jdbc:oracle:thin:@localhost:1521:XE  <-한번에 만들어도 됨
			
			String userName="kh_chj"; //사용자 계정명
			String password="kh1234"; //계정 비밀번호
			
			//****2-3)DB연결정보와 DriverManager를 이용해서 Connection 객체 생성
			...
			//연결된 DB에 SQL을 전달하고 결과를 반환받을 역할함
			
			//5.Statement 객체이용해 SQL 수행 후 결과반환받기
			
			//*****1)ResultSet Statement.executeQuery(sql);
			//->sql이 SELECT 문일때 결과로 ResultSet 객체 반환
			//2)int Statement.executeUpdate(sql);
			//->sql이 DML(INSERT/UPDATE/DELETE)일때 실행메서드
			...
			//커서를 다음 행으로 이동시킨 후
			//이동된 행에 값이 있음 true, 없음 false 반환
			//맨처음 호출시 1행부터 시작
			
			while(rs.next()){
				//자료형 ResultSet.get 자료형(컬럼명|순서);
				//현재 행에서 지정된 컬럼의 값을 얻어와 반환
				//->지정된 자료형 형태로 값이 반환됨
				//(자료형을 잘못 지정하면 예외 발생)
				
				//[java]			[DB]
				//String			CHAR, VARCHAR2, NVARCHAR
			...
			//7.사용완료된 jdbc 객체자원반환(close)
			//->자원반환 않으면 DB와 연결된 Connection이 그대로 남아
			//다른 클라이언트(ex.Java프로그램)가 추가적으로 연결되지 못하는 문제 발생할 수 있음!
			//->DBMS는 최대 Connection 수 갯수 제한하고 있기 때문에
